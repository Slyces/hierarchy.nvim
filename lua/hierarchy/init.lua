...
local ts_utils = RELOAD('nvim-treesitter.ts_utils')
local ClassNode = RELOAD('heritage.class_node')

local UpSearcher = RELOAD('heritage.upwards')
local DownSearcher = RELOAD('heritage.downwards')
...
function M.qf_up()
  local node = ts_utils.get_node_at_cursor()
  local class = ClassNode:from_children(node, 0)
  --reload('heritage.search').search(class, go_first_parent)
